...
import { View, Image, TextInput, StatusBar,ActivityIndicator, Dimensions, Platform , TouchableOpacity} from "react-native";
...
class Payment extends Component {
	...
	<StatusBar backgroundColor="#22313F" barStyle="light-content" />
				...
				<Header style = {{borderBottomColor: "#22313F", borderBottomWidth: 1, backgroundColor: "#22313F"}} >
					...
					<Body>
            <Title style = {{color: '#FFF',  fontFamily: 'Montserrat'}}>Payment</Title>

          </Body>
          <Right>
            <Button transparent
							onPress={() => this.props.navigation.navigate('CardView', { from: 'payment' })}
							>
							{this.getIcon(this.props.card_type)}
            </Button>
          </Right>
				...
				<Content>
					<Separator bordered style = {{
						...
						<ListItem
							button onPress={() => console.log('Cash')}>
						...
						<ListItem
								...
								<Text  style = {{
									 fontFamily: 'Montserrat'
								}}>{this.props.card_exist ? `**** **** **** ${this.props.last_4}` : 'CREDIT/DEBIT CARD'}</Text>
								<Right>
									{this.getIcon(this.props.card_type)}
								</Right>
						...
						<ListItem
							button onPress={() => this.props.navigation.navigate('Wallet' ,{from: 'payment'})}>

								<Text style = {{
									 fontFamily: 'Montserrat'
								}}>Fund Your Wallet ({this.props.wallet_nice}}</Text>
								<Right>
									<Icon name='add' />
								</Right>
						...
						<ListItem
							button onPress={() => this.setState({couponshow: true})}>
						...
						<Modal isVisible={this.state.couponshow}>
						<View style={{ flex: 0.5, 
							backgroundColor: '#FFF',
							borderRadius: 10,
							padding: 20,  
							...
							<TextInput
								...
								onChangeText = {(input)=>this.setState({coupon: input})}
							...
							<TouchableOpacity
							onPress = {() => this.props.coupon(this.state.coupon, this.props.user.userid)}
							style = {{
								marginTop: 60,
								alignSelf: 'center', width: '70%',
								borderRadius: 10,
								backgroundColor: '#22A7F0'}}>
								{this.props.getting_coupon ?
						...
						<TouchableOpacity
						onPress = {() => this.setState({couponshow: false})}
							>
							<Text style = {{
								color: '#FFF',
								fontSize: 18,
