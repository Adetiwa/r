...
import { View, Image, TextInput, NetInfo, StatusBar, StyleSheet, Dimensions, Platform , TouchableOpacity} from "react-native";
...
class Route extends Component {
    constructor(props) {
        ...
        this.state = {
            openLoader: this.props.getting_route || !this.props.network_connected,
            isLoading: this.props.getting_route,
  ...
  reload() {
	// if (this.props.network_connected || this.props.routes === null) {
	...
	render() {
		...
		<Header androidStatusBarColor="#22313F" style = {{borderBottomColor: "#22313F", borderBottomWidth: 1, backgroundColor: "#22313F"}} >
					<Left>
					  ...
					  <Icon style = {{color : "#FFF", fontSize: 40, fontFamily: 'Montserrat-Regular', zIndex: 100}}  size = {40} name= "close" />
                 </TouchableOpacity>

					</Left>

					<Body>
            <Title style = {{color: '#FFF',  fontFamily: 'Montserrat-Regular'}}>Routes</Title>
          ...
          <Right>
				...
				<Content style = {{
					...
					{this.props.getting_route === false &&
          ...
          <List
                      ...
                      renderRow={data =>
                    		<TouchableOpacity style = {{flex: 1, backgroundColor: '#FFF', padding: 20, marginTop: 10,
									}} 		 onPress={() => this.props.navigation.navigate('RouteSingle', { route_id: data.route_id })}>
										...
										<View style = {styles.row2}>
											<Text style = {{
									 			...
									 			<Text  style = {{
									 				 ...
									 				 color: '#777',
									 				 fontSize: 12,
									  }}>  ({data.stops.length} {data.stops.length > 1 ? 'Stops' : 'Stop'})</Text>
				...
				{/* <ProgressDialog 
				visible={this.props.getting_route} 
				message="Please, wait..."
                /> */}
